...
$lang->load('com_socialloginandsocialshare', JPATH_SITE);
  ...
  if ($manifest instanceof JSimpleXMLElement AND property_exists ($manifest, 'modules')) {
    if (is_array ($manifest->modules)) {
      foreach ($manifest->modules AS $mlvl1) {
        foreach ($mlvl1->children () as $module) {	    
		  $mod_data = array ();
	...
	if(!$isUpdate) {
	    $query = "UPDATE #__modules SET position='".$mod_data ['position']."', ordering='".$mod_data ['order']."', published=1, access=0 WHERE module='".$mod_data ['module']."'";
		$db->setQuery($query);
		$db->query();
    }
...
// Install plugin.
  if ($manifest instanceof JSimpleXMLElement AND property_exists ($manifest, 'plugins')) {
    if (is_array ($manifest->plugins)) {
      foreach ($manifest->plugins AS $plvl1)
        foreach ($plvl1->children () as $plugin) {
          $plg_data = array ();
          foreach ($plugin->attributes () as $key => $value) {
            $plg_data [$key] = strval ($value);
          }
          $this->parent->setPath ('extension_root', JPATH_ROOT . DS . 'plugins' . DS . $plg_data ['group'] );
          $created = false;
          ...
          if ($this->parent->parseFiles ($plugin->files[0], -1) === false) {
            $this->parent->abort ();
            return false;
          }
		  ...
		  $query = 'SELECT `id` FROM `#__plugins` WHERE folder = ' . $db->Quote ($plg_data ['group']) . ' AND element = ' . $db->Quote ($plg_data ['plugin']);
          $db->setQuery ($query);
          if (!$db->Query ()) {
          ...
          if (empty ($plugin_id)) {
            ...
            $plugin_data ['published'] = 1;
            $plugin_data ['ordering'] = $plg_data ['order'];
            $plugin_data ['iscore'] = 0;
            $plugin_data ['access'] = 0;
            $plugin_data ['params'] = '{}';
