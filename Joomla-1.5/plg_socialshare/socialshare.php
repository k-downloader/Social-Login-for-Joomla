    ...
    */
    public function __construct(&$subject)
    {
		...
		if($this->enableshare == '1'){
			...
			$document->addScript(JURI::root(true).'/plugins/content/socialshare.js');
            ...
            foreach ($this->horizontal_articles as $key=>$value) {
			  if ($article->id == $value) {
				...
				if(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']!='Off' && !empty($_SERVER['HTTPS'])):
						...
						if(!isset($article->sectionid) && empty($article->sectionid)):
							$article->sectionid = 0;
						endif;
						if(!isset($article->catid) && empty($article->catid)):
							$article->catid = 0;
						endif;
				$articlelink = $http.$_SERVER['HTTP_HOST'].JRoute::_(ContentHelperRoute::getArticleRoute($article->id, $article->catid, $article->sectionid));
